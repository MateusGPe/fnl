...
namespace mui
{
    ...
    public:
        ...
        {
            int x1 = std::clamp(static_cast<int>(src_px), 0, img_w - 1);
            int y1 = std::clamp(static_cast<int>(src_py), 0, img_h - 1);
            ...
            auto lerp_chan = [&](int offset) {
                float c11 = fg_data[idx11 + offset];
                float c21 = fg_data[idx21 + offset];
                float c12 = fg_data[idx12 + offset];
                float c22 = fg_data[idx22 + offset];
